...
| 입력 | 기능 |
|---|---|
| 왼쪽 스틱 | x-y 평면 이동 (조작 기준은 `operator_position` 참고) |
| L1 / R1 | z 상승 / 하강 (누르는 동안 — 왼스틱과 함께 "내려가며 이동" 가능) |
| 오른쪽 스틱 ←→ | yaw 회전 (오른쪽 = 시계방향, 상하축은 미사용) |
| D-pad | 편집할 틸트 성분·방향 선택 (↑↓=상하 성분, ←→=좌우 성분) — 선택만으로는 움직이지 않고 값도 유지됨 |
| ✕ (Cross) 탭 | 선택 방향으로 **현재 값에서** 30° 스텝 (눈금 스냅, 예: 74°→90°) |
| ✕ (Cross) 꾹 | 선택 방향으로 연속 미세 틸트 (기본 40°/s, 0.35s 이상 누르면) |
| ○ (Circle) 탭 | 활성 성분을 0° 쪽으로 30° 스텝 (예: 74°→60°) |
| ○ (Circle) 꾹 | 활성 성분 연속 복귀 (0°에서 정지) |
...
**조작 기준(`control.operator_position`)**: 기본값 `front`는 **로봇을 마주보고** 조작하는
기준으로, 스틱 x-y와 D-pad 틸트 방향이 미러링된다. 로봇 뒤에 서서 조작하면 `behind`로
바꾸면 된다 (yaw는 z축 회전이라 어느 기준이든 동일).

틸트는 상하/좌우 두 성분(각 ±90°, 설정 가능)의 합성으로, D-pad로 성분을 오가며 편집해도
값이 이어진다 — 예: 전방 60° 기운 상태에서 ↓ 선택 후 ✕ 탭 → 60°→30° (현재 값 기준).
복합 틸트(전방+측면)도 가능하며 ▢가 전체를 한 번에 리셋한다.
...
기본 입력 드라이버는 pydualsense(hidraw)로, 진동·어댑티브 트리거·라이트바·IMU를 지원한다
(`gamepad.driver: evdev`로 바꾸면 입력 전용 폴백). 피드백 채널:

| 채널 | 신호 | 의미 |
|---|---|---|
| 고주파 모터 | 타겟과 워크스페이스 박스 면과의 거리 | 경계 접근 경고 (5cm부터 램프) |
| 저주파 모터 | IK anti-windup `\|q_ref − q\|` 포화율 | 서보가 세게 밀고 있음 (누르기/막힘) |
| R2 트리거 저항 | 실측 그리퍼 폭 − 명령 폭 | 물체를 잡고 있음 (Rigid 저항) |
| 라이트바 | 녹화 상태 | 빨강=녹화 중, 파랑=대기 |

눌림 신호는 시뮬/실물 공통으로 동작하며(브리지 상태의 q vs 클라이언트 q_ref), 프로토콜 v2에서
`O_F_ext_hat_K` 기반의 실제 힘 단위로 업그레이드 예정. 파라미터는 `configs/teleop.yaml`의
`feedback` 섹션.
...
- [x] DualSense 햅틱 피드백 (경계 진동, 눌림 진동, 그립 트리거 저항, 녹화 LED)
- [ ] 카메라 관측 녹화 (RealSense) → LeRobot 포맷 변환
- [ ] IMU 활용 (패드 기울여 EE 미세 틸트, 흔들어 폐기 등 — GamepadState에 노출됨)
